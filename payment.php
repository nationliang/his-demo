...
echo "<form method=\"\">";
echo "<tr>";
echo "<th>序号</th><th>患者姓名</th><th>科室</th><th>医生姓名</th><th>挂号时间</th><th>挂号类型</th><th>费用</th>";
echo "</tr>";
...
if(count($rows)!=0){ 
 ...
 $sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and dep_id='$rows[id]'";
 $result=mysqli_query($link,$sql);
 ...
 $sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and dep_id='$rows[id]' limit $number,$pageamo";
...
if(count($rows2)!=0){ 
 $count--;
 $sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and d_id='$rows2[id]'";
 $result=mysqli_query($link,$sql);
 $row=mysqli_num_rows($result);
 if($row==0)
    $maxpage=1;
 ...
 if($_GET['nopage']){
  ...
  $nopage=1;
 $number=($nopage-1)*$pageamo;
 $sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and d_id='$rows2[id]' limit $number,$pageamo";
 $result=mysqli_query($link,$sql) or die(mysqli_error($link));
}
...
$sql2="select id from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and date='$stime'";
$result2=mysqli_query($link,$sql2);
$rows2=mysqli_fetch_array($result2);
if(count($rows2)!=0){
  $count--;
 ...
 if($_GET['nopage']){
  if($_GET['nopage']>$maxpage)
   ...
   $nopage=1;
 $number=($nopage-1)*$pageamo;
 $sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' and date='$stime' limit $number,$pageamo";
 $result=mysqli_query($link,$sql);	
...
$sql="select * from $table_patient where name='$_COOKIE[his_user]' and treat='yes' and pill='no' and pay='no' limit $number,$pageamo";
$result=mysqli_query($link,$sql);
